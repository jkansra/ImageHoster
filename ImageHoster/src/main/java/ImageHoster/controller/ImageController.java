...
public class ImageController {
    ...
    //According to the requirement, now the image is displayed by the title and the image id, as the image id is unique and even if the title
    //is same it would lead to no errors.
    //Added the imageId also as a path variable
    ...
    public String showImage(@PathVariable("id") Integer id,
        ...
        Image image = imageService.getImageByTitleAndImageId(title, id);
        List<Comment> comments = commentService.getCommentsForImage(image, user);
        if(comments == null) {
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, HttpSession session, Model model) {
        User user = (User) session.getAttribute("loggeduser");
        Boolean owner = imageService.deleteImage(imageId, user);
        if(!owner){
